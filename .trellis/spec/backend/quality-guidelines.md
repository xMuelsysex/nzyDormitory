...
- Root-relative static assets from the NJUCM CAS login page, such as `/js`, `/themes`, `/images`, and `/favicon.ico`, must resolve against the final CAS response host so the browser gets fetchable resource URLs; if WebVPN serves the CAS login HTML at `https://webvpn.njucm.edu.cn/login?...`, map those known static asset paths narrowly to `https://ids.njucm.edu.cn/...` because the WebVPN root can self-redirect those assets.
